		...
		<div id="boardList">
			<table> 
				<thead>
					<tr>
						<th class="no">No</th>
						<th class="title" style="text-align: center;">Booth Number</th> 
						<th class="date">Date</th>
						<th class="hit" width="150px">Hit</th> 
					</tr>
				</thead>
				<tbody>
					...
					?>
						...
						<tr class='<?php if($_GET["bno"] === $row['b_no']) echo "current-selected-item"; ?> hovering'>
							<td class="no"><?php echo $row['b_no']?></td>
							<td class="tietle">
								<a href="./view.php?bno=<?php echo $row['b_no']?>"><?php echo $row['b_title']?></a>
							</td> 
							<td class="date"><?php echo $row['b_date']?></td>
							<td width="100px" style="text-align: center;" class='<?php if($row["b_hit"] == 0) echo "Available"; ?>'> 
								<?php 
									if($row['b_hit'] == 1) {
										echo 'Not Available';
									} else {
